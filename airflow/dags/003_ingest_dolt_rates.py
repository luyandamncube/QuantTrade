...
# NOTE: your us_treasury table has only date + tenor columns; use date as PK
PK_MAP = {
    "us_treasury": ["date"],
}
...
def _safe_attr(name: str) -> str:
    # Mermaid attributes must start with letter/underscore; map bad names like '1_month' -> 'c_1_month'
...
def _infer_fk_edges_from_pk_map(tables, cols_by_table, pk_map):
    ...
    # actual PKs (with real casing) for tables present in pk_map
    pk_actual = {}
...
def generate_mermaid_erd(db_path: str, out_dir: str, erd_name: str, pk_map: dict | None = None) -> str:
    """DuckDB → Mermaid erDiagram
       ...
       - Draws declared FKs
       - Optionally infers edges from PK_MAP + peer links
    """
    ...
    # hard validation: no attribute starts with a digit
...
with DAG(
    ...
    # If you're on Windows without bash, you can swap this for a PythonOperator.
    ...
    def _render_mermaid_svg_py(**kwargs):
        ...
        try:
            # --- Defensive sanitize pass on the .mmd (in case a stale file is around) ---
            with open(mmd_path, "r", encoding="utf-8") as f:
                txt = f.read()

            # Replace any attribute lines like: "float 1_month" -> "float c_1_month"
            def _fix_line(m):
                t = m.group(1)
                name = m.group(2)
                fixed = name
            ...
            txt = re.sub(r"^(int|float|string|date|timestamp)\s+([^\s]+)",
                        _fix_line, txt, flags=re.M)

            # Write back the (possibly) fixed text
            with open(mmd_path, "w", encoding="utf-8") as f:
                f.write(txt)

            # Log first few lines to confirm what Mermaid will parse
    ...
    def _generate_pipeline_mermaid(**_):
        ...
        # Optional labels for branch edges (cosmetic)
        edge_labels = {
    ...
    def _render_pipeline_mermaid_svg(**kwargs):
        ...
        # reuse the same Python-only renderer pattern
        def ensure(pkg: str):
            try:
                return importlib.import_module(pkg)
            except ImportError:
